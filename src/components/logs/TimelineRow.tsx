...
const KIND_ICONS: Record<TimelineKind, { icon: LucideIcon; className: string }> = {
  ...
  closeDelayed: { icon: Timer, className: "text-amber-600 dark:text-amber-400" },
  simulatorReturned: { icon: Undo2, className: "text-emerald-600 dark:text-emerald-400" },
  simulatorStarted: { icon: PlaneTakeoff, className: "text-sky-600 dark:text-sky-400" },
  simulatorNotStarted: { icon: CircleAlert, className: "text-destructive" },
  keptByUser: { icon: Hand, className: "text-muted-foreground" },
  crashed: { icon: OctagonAlert, className: "text-destructive" },
  relaunched: { icon: RotateCcw, className: "text-emerald-600 dark:text-emerald-400" },
  error: { icon: CircleAlert, className: "text-destructive" },
  sessionEnded: { icon: Flag, className: "text-sky-600 dark:text-sky-400" },
};

function detailText(t: TFunction, entry: TimelineEntry): string | undefined {
  if (entry.kind === "closeDelayed" && entry.detail) {
    return t("logs.seconds", { seconds: entry.detail });
  }
  return entry.detail;
}
...
export function TimelineRow({ entry, language }: { entry: TimelineEntry; language: string }) {
  ...
  const detail = detailText(t, entry);
