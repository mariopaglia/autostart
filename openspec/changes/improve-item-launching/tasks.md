...
- [x] 4.2 Subir a versão para 0.2.0 em `package.json`, `src-tauri/Cargo.toml` e `src-tauri/tauri.conf.json`, datar o CHANGELOG, fazer o merge no `main` e, com a confirmação do mantenedor, enviar a tag `v0.2.0`. Verificar o release publicado com `AutoStart-Setup.exe`, `.sig` e um `latest.json` apontando para `AutoStart-Setup.exe`
